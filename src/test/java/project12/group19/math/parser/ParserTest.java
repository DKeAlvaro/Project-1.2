...
class ParserTest {
    ...
    public static Stream<Arguments> expressions() {
        return Stream.of(
                ...
                Arguments.of("sin ( max ( 2, 3 ) / 3 * pi )", Map.of(), 0),
                Arguments.of("0.5*(Math.sin((pi * (x-y))/7))+0.9", Map.of("x", 12.5, "y", 9.0), 1.4),
                Arguments.of("-0.5*-(--Math.sin((pi * (x-y))/7))+++0.9", Map.of("x", 12.5, "y", 9.0), 1.4),
                Arguments.of("ln(100) / ln(10)", Map.of(), 2.0),
                Arguments.of("-0.5 * +2", Map.of(), -1),
                Arguments.of("-(-2 * -3) * -4", Map.of(), 24)
        );
    }
    ...
    public void positive(String expression, Map<String, Double> variables, double expectation) {
        PostfixExpression parsed = SUT.parse(expression);
